 ...
 */
class VideoInterview extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return VideoInterview the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('FK_employer, FK_student, date, time, session_key, notification_id', 'required'),
			array('FK_employer, FK_student', 'numerical', 'integerOnly'=>true),
			array('session_key, notification_id', 'length', 'max'=>45),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, FK_employer, FK_student, date, time, session_key, notification_id', 'safe', 'on'=>'search'),
		);
	}
